import { LocalInternship, getLocalInternships } from './localStorage-internships';
...
function jaccardSimilarity(set1: string[], set2: string[]): number {
  ...
  const intersection = normalizedSet1.filter(x => normalizedSet2.includes(x)).length;
...
function semanticSectorBump(skills: string[], sector: string): boolean {
  const sectorKeywords: Record<string, string[]> = {
    ...
    analytics: ['python', 'data', 'analysis', 'machine learning', 'pandas', 'statistics'],
    design: ['figma', 'photoshop', 'illustrator', 'ui', 'ux', 'design'],
    finance: ['excel', 'finance', 'accounting', 'data analysis', 'math', 'financial modeling'],
...
function inclusionBonus(ruralFlag: boolean, sector: string): number {
  ...
  const ruralFriendlySectors = ['agriculture', 'education', 'public admin', 'civil engineering'];
...
function buildMatchReasons(
  ...
  const matchingSkills = candidateSkills.filter(skill =>
    internship.requiredSkills.some(req => req.toLowerCase() === skill.toLowerCase())
  );
...
export function getLocalRecommendations(profile: CandidateProfile): LocalRecommendationMatch[] {
  try {
    ...
    const scoredInternships = eligibleInternships.map(internship => {
      // Skills score (0-60)
      const skillScore = jaccardSimilarity(profile.skills, internship.requiredSkills) * 60;

      // Sector score (0-20)
      const sectorScore = sectorMatches(profile.sectorInterests, internship.sector)
        ? 20
