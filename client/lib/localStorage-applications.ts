...
export function saveLocalApplications(applications: LocalApplication[]): boolean {
  try {
...
export function addLocalApplication(
  ...
  try {
    ...
    const existingApp = existing.find(app => 
      app.profileId === profileId && app.internshipId === internshipId
    );
    ...
    if (success) {
      ...
      updateInternUsers(profileId, profile?.name || 'Unknown Candidate', profile?.email || 'unknown@example.com');
...
function updateInternUsers(profileId: string, name: string, email: string) {
  try {
    const internUsers = JSON.parse(localStorage.getItem(INTERN_USERS_KEY) || '[]');
    
    // Check if user already exists
    const exists = internUsers.find((user: any) => user.profileId === profileId);
