...
import { z } from "zod";
import { db } from "../db";
import { RecommendationsRequest, RecommendationsResponse, RecommendationMatch } from "@shared/api";
...
const RecommendationRequestSchema = z.object({
  ...
  profile: z.object({
    name: z.string(),
    educationLevel: z.enum(['TENTH_PLUS_TWO', 'DIPLOMA', 'UNDERGRADUATE', 'POSTGRADUATE']),
...
function locationMatches(candidatePin: string | null, candidateLocations: string[], 
                        internshipCity: string | null, internshipState: string | null, 
                        internshipPin: string | null, isRemote: boolean): number {
  ...
  for (const preferred of candidateLocations) {
    if (locationString.includes(preferred.toLowerCase()) || 
...
function sectorMatches(candidateInterests: string[], internshipSector: string): number {
  ...
  for (const interest of candidateInterests) {
    ...
    if (interest.toLowerCase().includes(normalizedSector) || 
        normalizedSector.includes(interest.toLowerCase())) return 0.6;
  }
...
function semanticSectorBump(skills: string[], sector: string): boolean {
  const sectorKeywords: Record<string, string[]> = {
    'it': ['javascript', 'python', 'java', 'programming', 'coding', 'software', 'web', 'react', 'node'],
    'healthcare': ['medical', 'health', 'biology', 'nursing', 'research', 'data analysis'],
...
function buildMatchReasons(candidateSkills: string[], candidateInterests: string[], 
                          candidateLocations: string[], candidatePin: string | null,
                          internship: any, skillMatches: number, sectorScore: number, 
  ...
  if (matchingSkills.length > 0) {
    reasons.push(`You have ${matchingSkills.length} of ${internship.requiredSkills.length} required skills: ${matchingSkills.slice(0, 3).join(', ')}`);
  }
  
  // Sector match
  if (sectorScore > 0.5) {
    reasons.push(`Strong match with your interest in ${internship.sector} sector`);
  }
  ...
  if (internship.remote) {
    ...
    reasons.push(`Located in your preferred area: ${internship.city}, ${internship.state}`);
  }
...
export const handleRecommend: RequestHandler = async (req, res) => {
  try {
    ...
    if (validatedData.profileId) {
      ...
      candidateProfile.sectorInterests = JSON.parse(candidateProfile.sectorInterests);
      candidateProfile.preferredLocations = JSON.parse(candidateProfile.preferredLocations);
    } else {
    ...
    const scoredInternships = eligibleInternships.map(internship => {
      ...
      // Skills score (0-60)
      const skillScore = jaccardSimilarity(candidateProfile.skills, requiredSkills) * 60;
      
      // Sector score (0-20)
      const sectorScore = sectorMatches(candidateProfile.sectorInterests, internship.sector) ? 20 : 
                         semanticSectorBump(candidateProfile.skills, internship.sector) ? 8 : 0;
      ...
      // Inclusion bonus (0-5)
      const inclusion = inclusionBonus(candidateProfile.ruralFlag, internship.sector);
      
      // Total score
      const totalScore = Math.min(100, skillScore + sectorScore + locationScore + inclusion);
